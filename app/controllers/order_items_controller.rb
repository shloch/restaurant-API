class OrderItemsController < ApplicationController
  ...
  def index
    render json: { results: OrderItem.where('shop_id' => @shopID).count }.to_json, status: :ok
  end

  def orders_by_sex
    itemsHash = { results: []}
    orders = OrderItem.where('shop_id' => @shopID).purchases_by_sex
  ...
  # classify spending by cities (percentage is with respect to entire products, not percentage per country)
  def orders_by_city
    itemsHash = { results: []}
    orders = OrderItem.where('shop_id' => @shopID).purchases_by_city
    if orders.size
      orders.each do |o|
  ...
  def highest_spender
    itemsHash = { results: []}
    spender = OrderItem.where('shop_id' => @shopID).highest_spender
  ...
  def spending_amounts_by_age_group
    itemsHash = { results: []}
    orders = OrderItem.where('shop_id' => @shopID).spending_by_age_group
    total_orders = OrderItem.where('shop_id' => @shopID).count
  ...
  def get_total_orders
    @total_orders = OrderItem.where('shop_id' => @shopID).count.to_f
  end

  def get_total_spent
    @total_spent = OrderItem.where('shop_id' => @shopID).total_spent.to_f
  end
